...
string convert_int_to_string_hexadecimal(int x,int fill = 5){
  ...
  string abhi_raj = s.str();
  abhi_raj = abhi_raj.substr(abhi_raj.length()-fill,fill);
  transform(abhi_raj.begin(), abhi_raj.end(),abhi_raj.begin(),::toupper);
  return abhi_raj;
}
...
class AbhiEvaluatedString{
public:
  int getResult();
  AbhiEvaluatedString(string data);
private:
  string storedData;
  int index;
  char peek();
...
AbhiEvaluatedString::AbhiEvaluatedString(string data){
  storedData = data;
  index=0;
}

// getresult function
int AbhiEvaluatedString::getResult(){
  int result = term();
  while(peek()=='+' || peek() == '-'){
    if(get() == '+'){
      result += term();
...
// factor function
int AbhiEvaluatedString::factor(){
  if(peek() >= '0' && peek() <= '9'){
    return number();
  }
  else if(peek() == '('){
...
int AbhiEvaluatedString::number(){
  int result = get() - '0';
  while(peek() >= '0' && peek() <= '9'){
...
// the index by one
char AbhiEvaluatedString::get(){
  return storedData[index++];
}

// peek function : returns the value at the present index
char AbhiEvaluatedString::peek(){
  return storedData[index];
}
