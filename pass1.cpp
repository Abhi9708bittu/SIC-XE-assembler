...
//Evaluates the resulting string value using an AbhiEvaluatedString object and stores the result in the abhiOperand output variable.
...
void Manipulate_EXPRESSION(string expression, bool& relative,string& abhi_rajOperand,int lineNumber, ofstream& errorFile,bool& error_flag){
  ...
  string valu_abhi=""; // initializing the string valu_abhi
  ...
  for(int i=0;i<expression.length();){
    ...
    if(SYMTAB[SINGLEOPERAND].exists=='y'){
      ...
      valu_abhi = to_string(convert_string_hex_to_int(SYMTAB[SINGLEOPERAND].address));
    }
    else if((SINGLEOPERAND != "" || SINGLEOPERAND !="?" ) && if_all_num(SINGLEOPERAND)){
      the_last_operand = 0;
      valu_abhi = SINGLEOPERAND;
    }
    else{
  ...
  if(!Illegal){
    if(count_of_pairs==1){
      relative = 1;
      AbhiEvaluatedString abhi_raj1OBJ(VALUESTRING);
      abhi_rajOperand = convert_int_to_string_hexadecimal(abhi_raj1OBJ.getResult());
    }
  ...
  while(opcode!="END"){
    if(!check_comment_line(fileLine)){
      ...
      if(OPTAB[REAL_OPCODE(opcode)].exists=='y'){
        if(OPTAB[REAL_OPCODE(opcode)].format==3){
          ...
          if(REAL_OPCODE(opcode)=="RSUB"){
            ...
            if(operand[operand.length()-1] == ','){
              read_first_non_white_space(fileLine,index,akshat_status_code,abhi_rajOperand);
              operand += abhi_rajOperand;
            }
          }

          if(FORMAT_FLAG(operand)=='='){
            abhi_rajOperand = operand.substr(1,operand.length()-1);
            if(abhi_rajOperand=="*"){
              abhi_rajOperand = "X'" + convert_int_to_string_hexadecimal(LOCATION_COUNTER-LAST_DELTA_LOCATION_COUNTER,6) + "'";
            }
            if(LITTAB[abhi_rajOperand].exists=='n'){
              LITTAB[abhi_rajOperand].value = abhi_rajOperand;
              LITTAB[abhi_rajOperand].exists = 'y';
              LITTAB[abhi_rajOperand].address = "?";
              LITTAB[abhi_rajOperand].blockNumber = -1;
            }
          }
        }
        ...
        int abhi_rajVariable;
        abhi_rajVariable = saveLOCATION_COUNTER;
        saveLOCATION_COUNTER = LOCATION_COUNTER;
        LOCATION_COUNTER = abhi_rajVariable;
        ...
        if(SYMTAB[operand].exists=='y'){
          ...
          error_flag = false;
          Manipulate_EXPRESSION(operand,relative,abhi_rajOperand,lineNumber,errorFile,error_flag);
          if(!error_flag){
            LOCATION_COUNTER = convert_string_hex_to_int(abhi_rajOperand);
          }
        ...
        if(operand=="*"){
          abhi_rajOperand = convert_int_to_string_hexadecimal(LOCATION_COUNTER-LAST_DELTA_LOCATION_COUNTER,6);
          relative = 1;
        }
        else if(if_all_num(operand)){
          abhi_rajOperand = convert_int_to_string_hexadecimal(stoi(operand),6);
          relative = 0;
        }
        else{
          char the_last_byte = operand[operand.length()-1];
        ...
        SYMTAB[label].name = label;
        SYMTAB[label].address = abhi_rajOperand;
        SYMTAB[label].relative = relative;
        SYMTAB[label].blockNumber = currentBlockNumber;
        LAST_DELTA_LOCATION_COUNTER = LOCATION_COUNTER - convert_string_hex_to_int(abhi_rajOperand);
      }
      else{
        read_first_non_white_space(fileLine,index,akshat_status_code,operand);
